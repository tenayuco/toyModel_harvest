...
colnames(DF_POST) = columns
  ...
  for (nP in unique(DF_NEW$numPlants)){
   ...
   # plot(plot_graph,layout = zippin,vertex.size = 1, vertex.label = NA,edge.arrow.size = 0.1,edge.color="red",edge.curved=0,vertex.color="black",edge.width=2,frame=T)
    ...
    DF_TEMP$N_REDES <- 2* numeroRedes/DF_TEMP$numPlants
...
FIG_REDES <- melt_DF_POST_RES %>% 
  ...
  geom_point(size= 1.5, aes(color = porcionCosecha))+
  geom_line(aes(color = porcionCosecha, linetype = porcionCosecha))+
  geom_errorbar(aes( 
    ymin=value_mean-value_sd, ymax=value_mean+value_sd, color = porcionCosecha)) +
