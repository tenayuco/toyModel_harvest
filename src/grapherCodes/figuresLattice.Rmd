...
FIG_STAT_REP <- DF_STATS_REP_L %>% 
 ...
 # theme(panel.spacing = unit(0.8, "lines"))+
  theme_bw()+
  facet_wrap(~Stat*numPlants, nrow = 2, scales = "free_y") +
  theme(text = element_text(size = 25))+
  theme(strip.background = element_rect(fill = "white"))+ 
  scale_alpha(guide = 'none')+
      labs(color= "Harvesting Scenario", x= "Number of repetitions", y= "Value")


ggsave(FIG_STAT_REP,filename=paste("../../output/graficas/SUP_FIG/", "meanSD_rep.png", sep=""),  height = 6, width = 16) # ID will be the unique identifier. and change the extension from .png to whatever you like (eps, pdf etc).
