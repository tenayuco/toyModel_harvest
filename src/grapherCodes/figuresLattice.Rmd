...
DF_SUPER_AG <- DF_TOTAL_AG%>%
  group_by(numPlants, numWorkers, DistanceW, porcionCosecha, SimID, Infection) %>%
...
for (nP in unique(DF_SUPER_AG$numPlants)){
  ...
  geom_violin(aes(fill= Infection), adjust = 3)+ 
  geom_point(aes(fill= Infection), color= "black", shape= 21, size= 2, alpha = 0.5)+
  theme_bw()+
    scale_fill_manual(values =colorsDis2)+
   scale_color_manual(values =colorsDis2)+
