...
LATTICE_RES <- LATTICE_DF %>%
  group_by(HarvestModel, Rep, Time, Rust)%>%
  summarise(Total_Sum = sum(Total)/N_PLANTS)

LATTICE_HAR <- LATTICE_DF %>%
  filter(Time==5)%>%
  filter(WorkerID !=0)%>%
  group_by(HarvestModel, Rep, WorkerID, HarvestEvent)%>%
...
FIG_SLI_time <- LATTICE_RES %>%
  ggplot()+
  geom_line(aes(x=Time , y= Total_Sum, group= interaction(Rep, Rust)), color="black")+ 
...
FIG_HAR_W <- LATTICE_HAR%>%
  ggplot()+
  geom_boxplot(aes(x= WorkerID, y= CosechaTotal, color= HarvestEvent))+ 
  ggtitle("")+
  scale_color_manual(values = mycols3c)+
  theme_bw()
