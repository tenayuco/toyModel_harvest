...
ggsave(FIG_PATH_2000_W1,filename=paste("../../output/graficas/PATH/", "path_plants_2000_1Worker.png", sep=""),  height = 10, width = 6) # ID will be the unique identifier. and change the extension from .png to whatever you like (eps, pdf etc).
...
DF_TOTAL_AG <- DF_TOTAL_AG %>%
  filter(!(is.na(MeanFrec)))

#DF_TOTAL_AG <- DF_TOTAL_AG %>%
 # group_by(numPlants, numWorkers,  Infection, DistanceW, porcionCosecha, SimID) %>%
#  summarise(MeanFrec =sum(Frec)/rep)

#and this last rep was included because sometimes no all the values were present im the 30 repetitions, rep =! REP!! and we need to assert that even if it was not there, 
#we have to divide it. This is not tje same as only 
...
DF_SUPER_AG <- DF_TOTAL_AG%>%
  ...
  summarise(MeanFrecTotal = sum(MeanFrec))
```
...
for (nP in unique(DF_SUPER_AG$numPlants)){
  ...
  theme(legend.position = "none")+
  xlim(0, 110)+
  labs(x= "Step length (m)", y= "Rust", fill= "Rust")
...
FIG_HIST_G_B <- DF_SUPER_TEMP %>%
  ...
  scale_alpha(guide = 'none')+
      labs(fill= "Rust", x= bquote("Step length (m)"), y= "Proportion of Steps")
...
HIS_COM_B<- FIG_HIST_G_B + annotation_custom(ggplotGrob(CAJADIST_B), xmin = 15, xmax = 100, ymin = 0.1, ymax = 0.4)
  ...
  #  ggsave(FIG_HIST_G,filename=paste("../../output/graficas/PATH/", "DisHIST_", "nP_", nP, "pC_", pC, ".png", sep=""),  height = 8, width = 14) # ID will be the unique identifier. and change the extension from .png to whatever you like (eps, pdf etc).
    ggsave(HIS_COM_B,filename=paste("../../output/graficas/PATH/", "DisFINAL_", "nP_", nP, "pC_", pC, ".png", sep=""),  height = 8, width = 10) # ID will be the unique identifier. and change the extension from .png to whatever you like (eps, pdf etc).
...
FIG_INF <- DF_TOTAL_AG %>%
  ...
  ggplot(aes(x= DistanceW)) +
    ...
    geom_ribbon(aes(ymin = FrecCUM - SDFrec, ymax = FrecCUM + SDFrec, fill= porcionCosecha), alpha= 0.7) +
    geom_line(aes(y= FrecCUM, group =SimID), color= "black", size=2)+
  geom_line(aes(y= FrecCUM, color=porcionCosecha, group =SimID), 
            size=1)+
  xlim(0, 30)+
  #ylim(0, 0.45)+
  facet_wrap(~numPlants, ncol = 1)+
...
#  theme(legend.position = "none")+
  ...
  scale_alpha(guide = 'none')+
     # labs(fill= "Rust", x= bquote('Step length' ~ (m^{1/2})), y= "Proportion of Steps")
      labs(fill= "Rust", x= 'Step length', y= "Proportion of Steps", color= "Harvesting scenario")


 ggsave(FIG_INF,filename=paste("../../output/graficas/DIF_RUST/", "INF_HAR.png", sep=""),  height = 6, width = 20)
...
FIG_W_DEN <- DF_SUPER_AG%>%
  ...
  geom_line(aes(group = porcionCosecha))+
   geom_errorbar(aes(ymin =MeanFrec- sdFrec , ymax = MeanFrec+ sdFrec), width = 0.2, color= "black")+ 
