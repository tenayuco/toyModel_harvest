...
##figuras sin el escenario de syncron Final
DF_SAM <- DF_SAM %>%
...
DF_SUPER_AG <- DF_TOTAL_AG%>%
  group_by(HarvestModel, numPlants, numWorkers, DistanceW, HarvestTime
           , porcionCosecha, SimID) %>%
  summarise(MeanFrecTotal = sum(MeanFrec))


DF_TOTAL_TEMP <- DF_SUPER_AG %>%
...
CAJADIST<- DF_TOTAL%>%
  ...
  geom_point(aes(), shape= 21, fill= "gray", color= "black", size= 2)+
  theme_bw()+
...
HIS_COM<- FIG_HIST_G + annotation_custom(ggplotGrob(CAJADIST), xmin = 15, xmax = 100, ymin = 0.1, ymax = 0.4)
